...
int runcolmap_with_align()
{
	...
	// Recon
		...
		incremental_mapper_options.min_num_matches = 15; // 15 for more points +100
...
int recon_scene()
{
	const fs::path input_root("H:/data/scene0000_00/data_20");
	...
	const fs::path intr_txt_path = input_root / "intrinsics_color.txt";		// 
	const std::string db_path = (output_root / "database.db").string();		// Canbe blank
	...
	// sparse recon
		...
		incremental_mapper_options.fix_existing_images = true;
		// Which intrinsic parameters to optimize during the reconstruction.
		/*incremental_mapper_options.ba_refine_focal_length = true;
		incremental_mapper_options.ba_refine_extra_params = true;
		incremental_mapper_options.ba_refine_principal_point = false;*/

		// recon option
		incremental_mapper_options.multiple_models = isMultipleModels;
		incremental_mapper_options.min_num_matches = 5; // 15 for more points +100
...
int recon_etu()
{
	const fs::path input_root("H:/Data/ETU/delivery_area");
	const fs::path output_root("H:/Data/ETU/delivery_area/colmap_recon");

	const std::string img_path = (input_root / "images" / "dslr_images_undistorted").string();	// Path of the input image
	//const fs::path pose_folder_path = input_root / "pose";									// c2w pose
	//const fs::path intr_txt_path = input_root / "dslr_calibration_undistorted" / "cameras.txt";	//
	const std::string db_path = (output_root / "database.db").string();							// Canbe blank
	...
	bool isSingleCamera = false;
	bool isGreaterBa = true;
	std::int16_t max_image_size = 6208;
	...
	// sparse recon exists, loading
	if (fs::exists(output_root / "sparse0")) {
		LOG(INFO) << "===== Existing Sparse Reconstruction, Dense Reconstruction Running =====";
		std::int16_t i = -1;
		// maybe sparse0,1,2.. exists
		while (fs::exists(output_root / ("sparse" + std::to_string(++i)))) {
			reconstruction_manager.Read((output_root / ("sparse" + std::to_string(i))).string());
